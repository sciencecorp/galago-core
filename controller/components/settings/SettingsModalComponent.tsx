        ...
        <ModalContent>
          ...
          <ModalBody>
            <VStack align="start" spacing={4}>
                ...
                <HStack>
                    <Box width='95%'>
                        <Select placeholder='Select option'>
                            {workcells && (workcells.map((workcell, index)=> (
                                <option key={index} value={workcell}>{workcell}</option>)
                            ))}
                        </Select>
                         {
              ...
              <FormLabel>Host Ip</FormLabel>
                <Input></Input>
                <FormHelperText>The computer running the app. Defaults to localhost if not provided. Required to access the app from the network.</FormHelperText>
              </FormControl>
              <FormControl>
                <FormLabel>Redis Host</FormLabel>
                <Input></Input>
                <FormHelperText>You must configure redis and specify url to run protocols.</FormHelperText>
              </FormControl>
              <FormControl>
                <FormLabel>Slack Bot Token</FormLabel>
                <Input></Input>
                <FormHelperText>Required to use slack messaging utils. Eg. errors, run start/end, etc.</FormHelperText>
              </FormControl>
              <FormControl>
                <HStack>
          ...
          <ModalFooter>
            <ButtonGroup>
              <Button
                colorScheme="blue"
                onClick={()=>{saveSettingsConfirmModal()}}>
                Save
              </Button>
