...
import { getRunAttributes, calculateRunTimes, groupCommandsByRun, calculateRunCompletion } from '@/utils/runUtils';
...
const RunQueueGanttChart: React.FC<GanttChartProps> = ({ onRunClick, selectedRunId }) => {
  ...
  useEffect(() => {
    ...
    const lastRunEndTime = allRuns.reduce((latest, run) => {
      const runEndTime = moment(
        run.completedAt || 
        moment(run.createdAt).add(600, 'seconds')
      );
      return runEndTime.isAfter(latest) ? runEndTime : latest;
    }, firstRunStartTime);

    const allCompleted = allRuns.every(run => 
  ...
  const renderRuns = () => {
    ...
    const allRuns = groupedCommands.map(group => 
      runsQuery.data.find(run => run.id === group.Id)
    ...
    const runBlocks = allRuns.map((run, index) => {
      ...
      const { runStart, runEnd, expectedDuration, isActive } = calculateRunTimes(runMetadata, currentTime);
      const completionPercentage = calculateRunCompletion(runCommands);
      
      const blockEndTime = isActive || runMetadata.status === "CREATED" 
        ? currentTime.clone().add(
            expectedDuration * ((100 - completionPercentage) / 100),
            'seconds'
          )
        : runEnd;
