...
import { Box, Button, Center, FormLabel, HStack, Text,Heading, Input, VStack,Spacer, useColorMode, useColorModeValue } from "@chakra-ui/react";
...
export const ScriptsEditor: React.FC<ScriptsEditorProps> = (props) => {
    const { code } = props;
    const [allScripts, setAllScripts] = useState<string[]>(["Test.py", "Test2.py", "Test3.py", "Test4.py", "Test5.py", "Test6.py", "Test7.py", "Test8.py", "Test9.py", "Test10.py"]);
    const [openTabs, setOpenTabs] = useState<string[]>(["Test.py", "Test2.py", "Test3.py", "Test4.py", "Test5.py"]);
    ...
    useEffect(() => {
        if (searchQuery) {
            setFilteredScripts(allScripts.filter((script) => script.toLowerCase().includes(searchQuery.toLowerCase())));
        } else {
        ...
        return (
            <HStack spacing={0} justifyContent="flex-start">
                {openTabs.map((tab, index) => (
                    ...
                    >
                        <Box flex="1" textAlign="left">{tab}</Box>
                        <CloseIcon 
                            fontSize={10} 
    ...
    return (
        <Box p={1} height="100%">
            <HStack justify="space-between" width="100%">
                ...
                <Button colorScheme="teal" leftIcon={<RiAddFill />}>New Script</Button>
            </HStack>
            ...
            <HStack spacing={2} padding={4} borderTop="1px solid" boxShadow="md" justifyContent="flex-end" bg="white" position="relative">
                <Button colorScheme="gray" size="sm">Save</Button>
                <Button colorScheme="teal" size="sm">Run</Button>
            </HStack>
        </Box>
    );
};
