...
export const NewToolModal: React.FC<AddToolCommandModalProps> = (props) => {
  ...
  const toolCardBg = useColorModeValue("gray.100", "gray.800");
  ...
  const ToolCard = ({ tool }: { tool: ToolType }) => {
    ...
    return (
      <Box 
        ...
        borderRadius="lg"
        cursor="pointer"
        bg={isSelected ? useColorModeValue("teal.50", "teal.900") : toolCardBg}
  ...
  return (
    <>
      <Button onClick={onSelectOpen} colorScheme="teal" leftIcon={<RiAddFill />} isDisabled={isDisabled}>
        New Tool
      </Button>
          ...
          <ModalBody>
            <VStack spacing={6} align="stretch">
              ...
              <Flex justify="space-between" align="center">
                <Text fontSize="md" fontWeight="bold">Available Tools</Text>
                {selectedTool && (
                  <HStack>
                    <Text fontSize="sm">Selected:</Text>
                    <Tag colorScheme="teal">{capitalizeFirst(selectedTool.replaceAll("_", " "))}</Tag>
                  </HStack>
                )}
              </Flex>
