...
dotenv.config();
const criticial_slack_channel = process.env.CRITICAL_CHANNEL || "C0672EF8QF6";
...
const slackClient = new WebClient("xoxb-2272312635361-5215741967345-qZK8sAMf9vkSLSY2WqpjNW2y");
export class CommandQueue {
  ...
  private async sendSlackNotification(message: string, threadTs?: string) {
    if (this.slackNotificationsEnabled) {
      try {
        const channel = criticial_slack_channel;
        if (!channel) throw new Error("Channel not found");
        const response = await slackClient.chat.postMessage({
          channel: channel,
          text: message,
          thread_ts: threadTs, // Specify the thread_ts parameter
        });
        return response;
      } catch (error) {
