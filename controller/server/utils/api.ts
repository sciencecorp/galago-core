...
export const unpackError = (error: any): string => {
  ...
  if (!error.response) {
    ...
    errorMessage = title && message ? `${title}: ${message}` : JSON.stringify(error.response.data.error);
...
export const post = async<T>(url: string, data: any): Promise<T|null> => {
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      throw new Error(`Error: ${error.response?.status} - ${errorMsg} - ${JSON.stringify(error.response?.data?.detail)}`);
    } else {
      console.error("Unexpected error:", error);
      errorToast("Unexpected error", "An unexpected error occurred");
      throw new Error("An unexpected error occurred");
