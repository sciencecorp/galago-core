...
export const OpentronsPassageParams = z.object({
  ...
  dissociation_plate_well_found: zConsumableWellPlateSelection(96).describe("The numeric index of wells containing reagents to consume").default(["A1", "A2", "A3", "A4", "A5", "A6"]),
...
export default class OpentronsPassageBaymax extends Protocol<typeof OpentronsPassageParams> {
  ...
  // check plate type and then make culturePlate type
  
  _generateCommands(params: z.infer<typeof OpentronsPassageParams>) {
    var cultureLabware = "6-well celltreat";
    if(!params.sourcePlates){
      params.sourcePlates = [];
    }
    if (params.sourcePlates[0].culturePlateType === "6 well") {
      cultureLabware = "6-well celltreat";
    }
    if (params.sourcePlates[0].culturePlateType === "384 well") {
        ...
        },
      {
        ...
        params: {
          ...
          params: {
            ...
            reagent_wells: params.dissociation_plate_well_found,
            well_array_to_process: plate.culturePlateWells,
          },
        },
      },
    ...
    for (let i = 0; i < params.sourcePlates.length; i++) {
      ...
      ...paths.liconicToOpentrons({labware:cultureLabware, location:"Nest5"}),
      {
        toolId: "pf400_1",
        toolType:  ToolType.pf400,
        command: "run_sequence",
      ...
      ...paths.opentronToLiconic({labware:cultureLabware, location:"Nest5"}),
      {
        label: "Load plate into Liconic",
        toolId: "liconic_1",
        toolType:  ToolType.liconic,
    ...
    commands.push(
      ...
      // params.sourcePlates[0].wellPlateIDs.forEach(sourceWell => {
      //   commands.push({
      //     label: "Posting note for plasmid well: " + sourceWell,
      //     toolId: "helix_tool_1",
      //     toolType: ToolType.helix_tool,
      //     command: "post_note_to_helix",
      //     params: {
      //       culture_id: params.destinationPlates[0].cultureIDs[0],
      //       well: params.destinationPlates[params.sourcePlates[0].wellPlateIDs.indexOf(sourceWell)],
      //       note: sourceWell.toString(),
      //     },
      //   });
      // });
