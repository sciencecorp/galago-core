...
def create_default_inventory_nests(db: Session, tool_id: int) -> list[inventory_models.Nest]:
    ...
    if not inventory_config:
        logger.info(f"Tool type '{tool.type}' is not in inventory map, skipping nest creation")
        return []
    
    rows, columns = inventory_config
    
    # Check if nests already exist for this tool
    existing_nests = db.query(inventory_models.Nest).filter(inventory_models.Nest.tool_id == tool_id).all()
    if existing_nests:
        logger.warning(f"Tool {tool.name} (id: {tool_id}) already has {len(existing_nests)} nests")
    ...
    logger.info(f"Created {len(created_nests)} inventory nests for tool '{tool.name}' ({rows}x{columns})")
    return created_nests


def recreate_inventory_nests(db: Session, tool_id: int, force: bool = False) -> list[inventory_models.Nest]:
    """
    ...
    if force:
        ...
        db.query(inventory_models.Nest).filter(inventory_models.Nest.tool_id == tool_id).delete()
        db.commit()
