...
#     "plate_type": "384 well", 
#     "percent_change": 100,
#     "well_array_to_process": [0,1,2,3,4,5,6,7,8],
#     "new_tip": False
# }
# params = {
#     "tiprack_wells":["A1","B1","C1","D1","E1","F1","G1","H1"],
...
params = {
    "tiprack_wells":["E5"],
    "reagent_wells": ["F6","G6","H6"],
    "plate_type": "96 well", 
    "percent_change": 50,
    "well_array_to_process": [17,18,19,25,26,27,33,34,35,42,43,44,50,51,57,58,59,65,66,67,73,74,75],
    "new_tip": True,
    "tipbox_slot":6
}
...
def run(
    ...
    #Now we need to change the offset based on the pipette head being used.
    #TODO: FIX THIS, both should use the same! I suspect its a calibration issue. Seems only the P1000 channel is calibrated.
    if plate_type == "384 well" or plate_type == "96 well":
        reagent_plate.set_offset(x=9.00, y=1.00, z=90)
    else:
        reagent_plate.set_offset(x=1.50, y=1.00, z=90)

    if plate_type=="6 well" or plate_type=="12 well" or plate_type=="24 well" or plate_type=="48 well" or plate_type=="6 well with organoid inserts" or plate_type=="96 well":
        tiprack_1000 = protocol.load_labware("opentrons_96_filtertiprack_1000ul", 6)
        tiprack_1000.set_offset(x=0.50, y=2, z=0.00)
        print("waste plate initialized")
        ...
        tiprack_200.set_offset(x=7.70, y=2.30, z=-1.00)
        ...
        waste_plate.set_offset(x=25, y=0.00, z=21)
        plate, addition_volume, removal_volume, removal_reps, tilted = initializePlate(plate_type=plate_type,percent_change=percent_change,protocol=protocol,plate_location=2)
    ...
    if (plate_type=="6 well" or plate_type=="12 well" or plate_type=="24 well" or plate_type=="48 well" or plate_type=="6 well with organoid inserts"):
        ...
        waste_plate.set_offset(x=-30.00, y=0.50, z=15.0)
        plate_type_offset(plate_type=plate_type,process_well_index=process_well_index,tilted=tilted, plate=plate,location=1)
        # Assuming 'waste_well' is a single well in your waste container where you want to consolidate the media
        starting_tip = tiprack_wells[0]
        if not p1000.has_tip:
            next_tip = tiprack_1000.next_tip(starting_tip=tiprack_1000.wells_by_name()[starting_tip])
            p1000.pick_up_tip(next_tip)
        p1000.consolidate(removal_volume, [plate.wells()[well] for well in well_array_to_process], waste_plate.wells()[0], new_tip='never')
        print("tilted is: ", tilted)
        # Reset well_count for reagent distribution
        well_count = 0
        for i in range(removal_reps):
            for j in range(len(well_array_to_process)):
                process_well_index = well_array_to_process[j]
                if not p1000.has_tip:
                    next_tip = tiprack_1000.next_tip(starting_tip=tiprack_1000.wells_by_name()[starting_tip])
                    p1000.pick_up_tip(next_tip)
                # Use integer division to determine reagent_index based on current well count
                reagent_index = reagent_wells[well_count // 10 % len(reagent_wells)]
                add_reagent(process_well_index=process_well_index, addition_volume=addition_volume,
                            reagent_index=reagent_index, plate_type=plate_type, starting_tip=starting_tip,
                            new_tip=new_tip, tilted=tilted, pipette=p1000, plate=plate,
                            reagent_plate=reagent_plate, tiprack=tiprack_1000, PLATE_OFFSET=PLATE_OFFSET)
                well_count += 1
        if new_tip is True and p1000.has_tip:
            p1000.drop_tip()
        ...
        p300.move_to(reagent_plate.wells_by_name()["A1"].top(5))
        #Removing media from the plate
        for i in well_array_to_process:
            if i % 16 == 0:
                p300.move_to(plate.wells()[0].top(10))
                p300.aspirate(removal_volume,plate.wells()[i])
                p300.aspirate(removal_volume,plate.wells()[i+1])
                p300.move_to(plate.wells()[0].top(35))
                p300.dispense(2*removal_volume,waste_plate.wells()[0])
                p300.blow_out(waste_plate.wells()[0])
...
# Initialize an index to keep track of the current reagent well
        ...
        for i in range(len(well_array_to_process)):
            ...
            if process_well_index % 16 == 0:
                ...
                p300.move_to(reagent_plate.wells_by_name()[current_reagent_well].top(5))
                # Aspirate 100 units of liquid from the current reagent well
                p300.aspirate(2*addition_volume, reagent_plate.wells_by_name()[current_reagent_well].bottom(1))
