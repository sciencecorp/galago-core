...
def plate_type_offset(plate_type: str,process_well_index: int, plate:PlateType,
                      tilted: bool, x1: float=0, y1: float=0, z1: float=0,
                      location: int=1) -> None:
    if plate_type == "6 well" and tilted is True:
        ...
        if process_well_index==0 or process_well_index==1: 
            plate.set_offset(x=11.00+x1, y=-6.00+y1, z=46.50+z1)
        if process_well_index == 2 or process_well_index == 3:
            plate.set_offset(x=7.55+x1, y=-6.00+y1, z=33.50+z1)
        if process_well_index == 4 or process_well_index == 5:
            plate.set_offset(x=5.00+x1, y=-6.00+y1, z=20.50+z1)
    elif plate_type == "6 well" and tilted is False:
        plate.set_offset(x=-10.00, y=-3.00, z=39.5)
...
def add_reagent(process_well_index: int,addition_volume: int,
    ...
    if not pipette.has_tip and new_tip is True and plate_type == "384 well":
        next_tip = tiprack.next_tip(starting_tip=tiprack.wells_by_name()[starting_tip])
        pipette.pick_up_tip(next_tip)       
    elif not pipette.has_tip and new_tip is False and plate_type != "384 well":
        next_tip = tiprack.next_tip(starting_tip=tiprack.wells_by_name()[starting_tip])
        pipette.pick_up_tip(next_tip)
    plate_type_offset(plate_type=plate_type,process_well_index=process_well_index,tilted=tilted, plate=plate,location=1)
    if plate_type == "384 well" and str(pipette) == "P300 8-Channel GEN2 on right mount":
        ...
        pipette.dispense(addition_volume, plate.wells()[process_well_index].top(-3))
...
def mix_liquid(process_well_index: int,mix_volume: int,plate_type: str,starting_tip: str,
        ...
        protocol:protocolType, new_tip: bool,mix_reps: int, tilted: bool) -> None:
    print("pipette.has_tip: ", pipette.has_tip)
    if not pipette.has_tip:
        next_tip = tiprack.next_tip(starting_tip=tiprack.wells_by_name()[starting_tip])
        pipette.pick_up_tip(next_tip)
    if tilted is True:
        tilt(1, "up", pipette=pipette, protocol=protocol)
    for i in range(mix_reps):
        plate_type_offset(plate_type=plate_type,process_well_index=process_well_index, plate=plate, tilted=tilted,location=1)
        if plate_type == "6 well":
            pipette.move_to(plate.wells()[process_well_index].top(40))
            pipette.aspirate(mix_volume, plate.wells()[process_well_index])
            x_offset = -25
            y_offset = -15+5*i
            z_offset = 14 + 1.5*i
            ...
            plate_type_offset(plate_type=plate_type,process_well_index=process_well_index, plate=plate, tilted=tilted,location=1,x1=x_offset,y1=y_offset,z1=z_offset)
            pipette.dispense(mix_volume, plate.wells()[process_well_index])
        else:
...
def run(
    ...
    plate, addition_volume, removal_volume, removal_reps, tilted,new_tip = initializePlate(protocol=protocol, plate_type=plate_type, percent_change=percent_change,plate_location=1)
    
    mix_reps = 5
    mix_volume = addition_volume
    well_count = 0
    if plate_type == "384 well":
        new_tip = True
        mix_volume = addition_volume/2
    for i in range(len(well_array_to_process)):
        starting_tip = tiprack_wells[well_count]
        process_well_index = well_array_to_process[i]
    ...
    if plate_type == "6 well":
        for i in range(len(well_array_to_process)):
            process_well_index = well_array_to_process[i]
            mix_liquid(process_well_index=process_well_index,
                    mix_volume= mix_volume,plate_type=plate_type, protocol=protocol,
                    starting_tip=tiprack_wells[0],tiprack=tiprack_1000,
                    pipette=p1000,plate=plate,PLATE_OFFSET=PLATE_OFFSET, 
                    new_tip=new_tip,mix_reps=mix_reps,tilted=tilted)
    elif plate_type == "96 well" or plate_type == "384 well":
        for i in range(len(well_array_to_process)):
            process_well_index = well_array_to_process[i]
            mix_liquid(process_well_index=process_well_index,
                    mix_volume= mix_volume,plate_type=plate_type, protocol=protocol,
                    starting_tip=tiprack_wells[i],tiprack=tiprack_1000,
                    pipette=p1000,plate=plate,PLATE_OFFSET=PLATE_OFFSET, 
                    new_tip=new_tip,mix_reps=mix_reps,tilted=tilted)
            if plate_type == "384 well":
                #well count doesn't change
                well_count = well_count
            else:
                well_count = well_count + 1  
    if tilted is True and plate_type == "6 well":
